...
def _equal(n1, n2):
    ...
    truth = nx.is_isomorphic(int_n1, int_n2, node_match=nm)
    return truth
...
class ReactionNetwork:
    ...
    def _place_into(self, unit: str, n_info: List[Union[str, float, float]]):
    ...
    def parse_rule(self, line, params):
        ...
        if len(items) > 2:
            ...
            k_off = 0
        self.allowed_edges[tuple(sorted([r_info[0], r_info[1]]))] = [k_on, k_off]
    ...
    def match_maker(self, n1, n2=None) -> list:
        """
        determines if a valid edge can be added between two network states, and preforms addition if possibl
        :param n1: node in network
        :param n2: node in network
        :return:
        """
        # TODO: Theres an issue where sometimes a disconnected struct gets added. Needs fixing.
        nodes_added = []
        orig = n1[1]['struct']
        if n2 is not None:
            nextn = n2[1]['struct']
        ...
        for poss_edge in list(self.allowed_edges.keys()):
            if False not in [item.has_node(n) for n in poss_edge] and \
                    (n2 is None or
                     (True in [orig.has_node(n) for n in poss_edge] and True in [nextn.has_node(n) for n in poss_edge]))\
                    and not item.has_edge(poss_edge[0], poss_edge[1]):
                connected_item = item.copy()
                connected_item.add_edge(poss_edge[0], poss_edge[1])
            else:
                continue
            node_exists = [x for x in self.network.nodes(data=True) if _equal(x[1]['struct'], connected_item)]
...
if __name__ == '__main__':
    ...
    m = ReactionNetwork(sys.argv[1], dt, iter)
    print('done')
